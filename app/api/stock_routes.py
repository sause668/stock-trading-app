...
def get_user_stocks():
    ...
    return jsonify([{
        ...
        "amount": stock.amount,
        "price": stock.price,
        "previousPrice": requests.get(
            f'https://api.polygon.io/v1/open-close/{stock.name}/{yesterday}?adjusted=true&apiKey=YOUR_API_KEY'
        ).json().get('afterHours', 0.0)
    } for stock in stocks])
...
def buy_stocks(symb):
    ...
    portfolio = Portfolio.query.filter_by(user_id=current_user.id).first()
    if portfolio.money < (amount * price):
        return jsonify({"message": "Insufficient funds"}), 400

    # Deduct from balance
    portfolio.money -= (amount * price)

    # Add stock and transaction
    stock = Stock(name=symb, portfolio_id=portfolio.id, amount=amount, price=price)
...
def sell_stocks(symb):
    ...
    # Credit to balance
    portfolio.money += (amount * sale_price)

    # Reduce or delete stock
    stock.amount -= amount
    if stock.amount <= 0:
